...
Token LexicalAnalyzer::ScanNumber()
{
    ...
    if (c == '0') {
        ...
        for (int i = 0; i < 3; i++) {
            input.GetChar(c);
            if (input.EndOfInput() || c != "x08"[i]) {
                if (!input.EndOfInput()) {
                    input.UngetChar(c);
                }
                for (int j = 0; j < i; j++) {
                    input.UngetChar("x08"[i - j - 1]);
                }
                match = false;
                break;
            }
        ...
        input.GetChar(c);
        while (!input.EndOfInput() && c >= '0' && c <= '7') {
            tmp.lexeme += c;
            input.GetChar(c);
        }
        if (!input.EndOfInput()) {
    ...
    if (c == '0') {
        ...
        for (int i = 0; i < 3; i++) {
            input.GetChar(c);
            if (input.EndOfInput() || c != "x16"[i]) {
                if (!input.EndOfInput()) {
                    input.UngetChar(c);
                }
                for (int j = 0; j < i; j++) {
                    input.UngetChar("x16"[i - j - 1]);
                }
                match = false;
                break;
            }
